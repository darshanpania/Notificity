...
/**
 * Wrapper around [AnalyticsService] to log analytics events. Simplifies event logging by providing
 * easy-to-use functions for typical app actions.
 */
object AnalyticsLogger {
    ...
    fun onAppLaunch(source: String) {
        AnalyticsService.logEvent(
            object : AnalyticsEvent {
                ...
                override val properties = mapOf(AnalyticsConstants.Params.SOURCE to source)
            })
    }
    ...
    fun onScreenViewed(screenName: String, screenClass: String) {
        AnalyticsService.logScreen(
            ...
            properties = mapOf(AnalyticsConstants.Params.SCREEN_CLASS to screenClass))
    }
        ...
        AnalyticsService.logEvent(
            object : AnalyticsEvent {
                ...
                override val properties =
                    mapOf(AnalyticsConstants.Params.PERMISSION_STATUS to status.code)
            })
    }

    /**
     * Sets the user's current notification permission status as a user property. Enables filtering
     * or segmentation based on permission status in analytics.
     */
    fun setNotificationPermissionProperty(status: NotificationPermissionStatus) {
        AnalyticsService.setUserProperty(
            AnalyticsConstants.Params.PERMISSION_STATUS, status.code.toString())
    }
    ...
    fun onThemeToggleClicked(theme: String) {
        AnalyticsService.logEvent(
            object : AnalyticsEvent {
                ...
                override val properties = mapOf(AnalyticsConstants.Params.THEME to theme)
            })
    }
    ...
    fun onContributorProfileClicked(name: String) {
        AnalyticsService.logEvent(
            object : AnalyticsEvent {
                ...
                override val properties = mapOf(AnalyticsConstants.Params.CONTRIBUTOR_NAME to name)
            })
    }
    ...
    /** Logs user login event. Also sets user ID and user type property on the analytics service. */
