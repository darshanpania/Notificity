...
class ConstantsTest {
    ...
    fun `getLabel returns correct string for 7 days`() {
        ...
        val label = Constants.RetentionPeriod.getLabel(mockContext, Constants.RetentionPeriod.DAYS_7)
    ...
    fun `getLabel returns correct string for 30 days`() {
        ...
        val label = Constants.RetentionPeriod.getLabel(mockContext, Constants.RetentionPeriod.DAYS_30)
    ...
    fun `getLabel returns correct string for unlimited`() {
        ...
        val label = Constants.RetentionPeriod.getLabel(mockContext, Constants.RetentionPeriod.UNLIMITED)
        assertEquals("Unlimited", label)
    }

    @Test
    fun `getLabel returns unlimited string for unknown period`() {
        `when`(mockContext.getString(R.string.setting_retention_unlimited)).thenReturn("Unlimited") // Default fallback
